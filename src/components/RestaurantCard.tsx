...
const RestaurantCard: React.FC<RestaurantCardProps> = ({ restaurant }) => {
  ...
  return (
    <div 
      className={`overflow-hidden cursor-pointer rounded-lg shadow-sm border border-gray-200 bg-white ${isMobile ? '' : 'h-full'}`}
      onClick={handleClick}
    >
      <div className="relative overflow-hidden">
        <img 
          ...
          alt={restaurant.name}
          className={`w-full ${isMobile ? 'aspect-[16/9]' : 'aspect-[4/3]'} object-cover`}
        />
        {!isMobile && (
